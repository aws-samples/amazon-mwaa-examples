...
const params = {
  "Name": CONFIG.mwaaEnvironmentName
};
const command = new CreateWebLoginTokenCommand(params);
...
export const handler: CloudFrontRequestHandler = async (event) => {
  ...
  try {
      ...
      const response = {
        ...
        headers: {
          ...
          "set-cookie": [
            {
              key: "set-cookie",
              value: `auth-edge-nonce=${encodeURIComponent(nonce)}; ${
                CONFIG.cookieSettings.nonce
              }`,
            },
            {
              key: "set-cookie",
              value: `auth-edge-nonce-hmac=${encodeURIComponent(
    ...
    CONFIG.logger.debug("uri:\n", uri);
    CONFIG.logger.debug("queryString:\n", querystring);

    // Get MWAA web token
    const mwaaWebToken = await getMwaaWebToken();

    const success_response = {
      status: "302",
      statusDescription: "Found",
      headers: {
        location: [{key: "location", value: `https://${CONFIG.mwaaPublicEndpoint}/aws_mwaa/aws-console-sso?login=true#${mwaaWebToken}`}]
      }
    };

    // Return the request with token to allow access to the resource:
    CONFIG.logger.debug("Returning success_response:\n", success_response);
    ...
    const response = {
      ...
      headers: {
        ...
        "set-cookie": [
          {
            key: "set-cookie",
            value: `auth-edge-nonce=${encodeURIComponent(state.nonce)}; ${
              CONFIG.cookieSettings.nonce
            }`,
          },
          {
          ...
          {
            key: "set-cookie",
            value: `auth-edge-pkce=${encodeURIComponent(state.pkce)}; ${
              CONFIG.cookieSettings.nonce
            }`,
          },
        ],
...
async function getMwaaWebToken() {
    try {
      const data = await mwaa.send(command);
      CONFIG.logger.debug("MWAA Token retrieved:", data.WebToken);
      return data.WebToken;
    } catch (error) {
      //const { requestId, cfId, extendedRequestId } = error.$metadata;
      //CONFIG.logger.error({ requestId, cfId, extendedRequestId });
