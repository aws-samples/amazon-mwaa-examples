...
const US_EAST_1_STACK_BASE_TEMPLATE = JSON.stringify({
  Description: [
    "CloudFront with Cognito authentication using Lambda@Edge to enable custom domain for public MWAA endpoint.",
  ...
  await Promise.all(
    LAMBDA_NAMES.map((lambdaName) => {
      ...
      return copyLambdaCodeToUsEast1({
        ...
        updatedLambdaResource.Properties.Role = lambdaName == "CheckAuthHandler" ? props.checkAuthLambdaRoleArn : props.lambdaRoleArn;
...
export const handler: CloudFormationCustomResourceHandler = async (event) => {
  ...
  const {
    ...
    ResourceProperties: {
      ...
      CheckAuthLambdaRoleArn: checkAuthLambdaRoleArn,
